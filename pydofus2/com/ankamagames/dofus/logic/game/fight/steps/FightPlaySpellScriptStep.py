...
class FightPlaySpellScriptStep(AbstractSequencable, IFightStep):
    ...
    def __init__(self, contexts: list[SpellScriptContext], spellCastSequence: ISpellCastSequence, spellRank: int):
        super().__init__()
        self._casterIds = []
        if not contexts:
            return
    ...
    def handleFightContext(
        self, context: SpellScriptContext, spellRank: int, spellCastSequence: ISpellCastSequence
    ) -> None:
        ...
        if not spellLevelData or not spellLevelData.playAnimation:
            return
        Logger().debug(f"Fighter {context.casterId} Casting Spell '{spellData.name}' ({spellData.id})")
