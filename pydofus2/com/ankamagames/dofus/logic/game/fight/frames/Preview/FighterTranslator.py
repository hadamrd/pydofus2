...
from pydofus2.com.ankamagames.dofus.network.types.game.context.fight.GameFightCharacterInformations import GameFightCharacterInformations
from pydofus2.com.ankamagames.dofus.network.types.game.context.fight.GameFightEntityInformation import GameFightEntityInformation
...
class FighterTranslator(HaxeFighter):
    BOMBS_TYPE_ID = 95
    UPDATED_STATS = [
        "PUSH_DAMAGE_BONUS", "PUSH_DAMAGE_REDUCTION", "WATER_ELEMENT_RESIST_PERCENT",
        "EARTH_ELEMENT_RESIST_PERCENT", "FIRE_ELEMENT_RESIST_PERCENT", "AIR_ELEMENT_RESIST_PERCENT",
        "NEUTRAL_ELEMENT_RESIST_PERCENT", "RECEIVED_DAMAGE_MULTIPLIER_DISTANCE"
    ]
    ...
    def __init__(self, fighterInfos:GameFightFighterInformations, fighterId, isSummondCastPreviewed=False):
        ...
        super().__init__(fighterId, level, self.getBreed(), self.getPlayerType(), self._fighterInfos.spawnInfo.teamId, self.isAStaticElement(fighterId), buffs, data, isSummondCastPreviewed)
    ...
    def initializeBuffs(self, fighterId: float) -> List:
        unknownBaseStats = not (CurrentPlayedFighterManager().currentFighterId == fighterId or PlayedCharacterManager().id == fighterId or isinstance(self._fighterInfos, GameFightMonsterInformations))
        buffs = []
        for buff in BuffManager().getAllBuff(fighterId):
            if not (isinstance(buff, StatBuff) and not buff.isRecent and unknownBaseStats):
                if not ((unknownBaseStats or isinstance(self._fighterInfos, GameFightMonsterInformations)) and ActionIdHelper.getActionIdStatName(buff.actionId) in self.UPDATED_STATS):
                    if buff.stack:
                        for stack in buff.stack:
                            if stack.effect.delay == 0 and (not isinstance(buff, StateBuff) or FightersStateManager().hasState(fighterId, buff.stateId)):
                                buffs.append(DamagePreview.createHaxeBuff(stack))
                    elif buff.effect.delay == 0 and (not isinstance(buff, StateBuff) or FightersStateManager().hasState(fighterId, buff.stateId)):
